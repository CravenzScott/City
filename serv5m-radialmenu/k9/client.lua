...
--[[ NUI Callbacks ]]--
    ...
    RegisterNetEvent('k9:summon')
    AddEventHandler('k9:summon', function()
    ...
    RegisterNetEvent('k9:vehicletoggle')
    ...
    RegisterNetEvent('k9:dismiss')
    AddEventHandler('k9:dismiss', function()
        EnableMenu()
    end)
    ...
    RegisterNetEvent('k9:vehiclesearch')
...
--[[ Main Event Handlers ]]--
    ...
    AddEventHandler("K9:ToggleK9", function(model)
        if spawned_ped == nil then
            ...
            AddTextComponentString(tostring("LSPD: ".. k9_name))
        ...
        if spawned_ped ~= nil then
            if not following then
                ...
                if has_control then
                    ...
                    following = true
                    attacking = false
                    exports['mythic_notify']:SendAlert('inform', 'K9 is following you!')
                ...
                if has_control then
                    ...
                    following = false
                    attacking = 
                    exports['mythic_notify']:SendAlert('error', 'K9 stood where he was!')
                end
            end
        end
    end)
    ...
    AddEventHandler("K9:ToggleVehicle", function(isRestricted, vehList)
        if not searching then
            if IsPedInAnyVehicle(spawned_ped, false) then
                ...
                exports['mythic_notify']:SendAlert('error', 'K9 got out of the car!')
            else
                    ...
                    if door ~= false then
                        if isRestricted then
                            if CheckVehicleRestriction(vehicle, vehList) then
                                TaskEnterVehicle(spawned_ped, vehicle, -1, door, 1.0, 1, 0)
                                exports['mythic_notify']:SendAlert('success', 'K9 got into the vehicle!')
                            end
                        else
                            TaskEnterVehicle(spawned_ped, vehicle, -1, door, 1.0, 1, 0)
                            exports['mythic_notify']:SendAlert('success', 'K9 got into the vehicle!')
    ...
    AddEventHandler("K9:SearchVehicle", function(items, openDoors)
        ...
        if vehicle ~= 0 and not searching then
            ...
            exports['mythic_notify']:SendAlert('inform', 'The K9 is all about searching!')
            ...
            exports['mythic_notify']:SendAlert('error', 'What K9 found: ' .. tostring(table.concat(stringified_table)))
            searching = false
        end
    end)

    RegisterNetEvent("k9:searchplayer")
    AddEventHandler("k9:searchplayer", function(items)
        local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
        Citizen.Trace(tostring(closestPlayer))
        Citizen.Trace(tostring(json.encode(items)))
        if closestPlayer ~= -1 and not searching and closestDistance <= 3.0 then
            searching = true
            local found_table = {}
            local coords = GetEntityCoords(closestPlayer)
            exports['mythic_notify']:SendAlert('inform', 'K9 started searching for the person!')
            TaskGoToCoordAnyMeans(spawned_ped, coords.x, coords.y, coords.z, 5.0, 0, 0, 1, 10.0)
            local fourItem = ChooseItem(items)
            if fourItem ~= false then
                table.insert(found_table, fourItem)
            end
            ...
            exports['mythic_notify']:SendAlert('error', 'There is no one nearby!')
        end
    end)
...
--[[ Threads ]]
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            -- Trigger Opens Menu
            -- if IsControlPressed(1, 61) and IsControlJustPressed(1, 74) then
